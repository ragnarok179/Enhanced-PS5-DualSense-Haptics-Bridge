...
func enumerateBluetoothDualSense() ([]*device, error) {
	var hg guid
	...
	for index := uint32(0); ; index++ {
		...
		if inLen < 78 || outLen < bluetoothAudioReportSize {
			procClose.Call(uintptr(h))
			continue
		}
		out = append(out, &device{handle: h, path: path, product: productString(h), outputLen: outLen, inputLen: inLen})
	}
	return out, nil
}
func findBluetoothDualSense() (*device, error) {
	ds, err := enumerateBluetoothDualSense()
	if err != nil {
		return nil, err
	}
	if len(ds) == 0 {
...
func (d *device) writeReport(report []byte) error {
	...
	// The target Windows HID collection advertises 547 output bytes. The only
	// 0x32 test physically confirmed on this controller used the logical Sony
	// packet at the beginning of a zero-padded OutputReportByteLength buffer.
	// Reproduce that route deliberately for both 0x31 and 0x32. The CRC remains
	// at the logical protocol offset and the padding stays zero.
	wire := report
...
func enableRealtimeScheduling() func() {
	// The exact 142-byte 0x32 stream has a 10.67 ms packet period. Request 1 ms timer
