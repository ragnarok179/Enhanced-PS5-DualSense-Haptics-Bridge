...
// applyABSHybridPulse reproduces the stronger validated ABS feel used before
// the progressive-overlay experiment: a short release, a strong 7/8 kick and
// a light 1/8 hydraulic base. Strength is represented internally as 0..255 and
// quantized only when the final DualSense report is built. The user ABS setting
// scales the kick while the L2 brake setting scales the light base preload.
func applyABSHybridPulse(t telemetry, now time.Time, state *absPulseState) (telemetry, bool, string, int, int, int, float64) {
	...
	if rawActive {
		...
		state.lastStrong = t.Raw.ABSSeverity >= 0.35 || t.Raw.ABSWheelCount >= 1 || t.L2Amplitude >= strength8To255(4)
	...
	settings := currentUserSettings().AdaptiveTriggers
	profile := feelProfile().Triggers.ABS
	base := scaleNormalized255(clampInt(profile.BaseStrength255, 1, 255), settings.L2BrakePercent)
	kick := scaleNormalized255(clampInt(profile.KickStrength255, 1, 255), settings.ABSPercent)
	base = clampInt(base, 1, 255)
	kick = clampInt(kick, 1, 255)
	...
	if phaseElapsed < releaseDuration {
		if state.lastLock {
			...
			return t, true, "lock_base", base, kick, kick, cadenceHz
		}
		t.L2Mode = 0
		t.L2StartZone = 0
		t.L2StartStrength, t.L2EndStrength = 0, 0
		t.L2Amplitude, t.L2Hz = 0, 0
		return t, true, "release_off", base, kick, 0, cadenceHz
	}
	...
	t.L2StartStrength, t.L2EndStrength = base, base
	t.L2Amplitude, t.L2Hz = 0, 0
	return t, true, "base", base, kick, base, cadenceHz
}
...
func applyAdaptiveThrottleTrigger(t telemetry, now time.Time, state *adaptiveThrottleTriggerState, shiftTorqueCut bool) (telemetry, bool, float64, float64, float64) {
	...
	// No slip: keep the validated constant accelerator resistance on the normalized 0..255 scale.
	state.currentPosition = 0
	state.currentStrength = 0
	t.R2Mode = 1
	t.R2StartZone = 0
	cfg := feelProfile().Triggers.R2
	t.R2StartStrength = clampInt(cfg.NormalStartStrength255, 1, 255)
	t.R2EndStrength = clampInt(cfg.NormalEndStrength255, t.R2StartStrength, 255)
	t.R2Amplitude, t.R2Hz = 0, 0
	return t, false, 0, 0, 0
}
