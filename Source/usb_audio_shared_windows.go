...
type hapticAudioEngine struct {
	...
	speaker      *controllerSpeakerEngine
}
...
func (h *hapticAudioEngine) renderLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			speakerQueued := h.speaker != nil && h.speaker.hasUSBPCM()
			if queued <= 0 && !speakerQueued {
				h.mu.Unlock()
				continue
			}
			if queued > 0 && !speakerQueued && uint32(queued) < available {
			...
			if h.speaker != nil && h.speaker.renderUSB(dst, int(available), h.channels, h.blockAlign, h.floatPCM) {
				silent = false
			}
			flags := uintptr(0)
			if silent {
