...
func enumerateBluetoothDualSense() ([]*device, error) {
	...
	for index := uint32(0); ; index++ {
		...
		outLen, inLen, featureLen := 547, 78, 64
		if capsOK {
		...
		out = append(out, &device{handle: h, path: path, product: productString(h), productID: attrs.ProductID, outputLen: outLen, inputLen: inLen, featureLen: featureLen})
	}
	return out, nil
}
...
func (d *device) writeReport(report []byte) error {
	...
	// Some HID collections advertise a larger OutputReportByteLength than the
	// logical Sony control report. Pad only when the Windows HID route requires it;
	// the protocol CRC remains at its logical offset and padding stays zero.
	wire := report
